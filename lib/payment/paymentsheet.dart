...
import 'package:sdkapp/payment/latpay_3ds_screen.dart.dart';
import 'package:sdkapp/payment/latpay_default_result.dart';
import 'package:sdkapp/payment/latpay_payment_action_callback.dart';

import 'input_formatters.dart';
import 'card_utils.dart';
import 'latpay_service.dart';

import 'latpay_crypto.dart';
import 'latpay_payment_service.dart';
        ...
        children: [
          ...
          */
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: Color.fromRGBO(37, 51, 113, 1),
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(6),
                ),
              ),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: () {},
              child: Text("Pay with ${widget.cardToken}"),
            ),
          ),

          const SizedBox(height: 18),
          const Text("---------- Or Pay with Card ----------",
              style: TextStyle(color: Colors.grey)),
  ...
  */
  ...
  Future<void> _processPayment() async {
    try {
      ...
      final cardNameParts = _nameCtrl.text.trim().split(RegExp(r'\s+'));
      final payload = {
        "merchantid": widget.merchantId,
        "publickey": widget.publicKey,
        "transactionkey": transKey,
        ...
        "currency": widget.currency,
        "reference": "test00001",
        "description": "SkyRich Test",
        "billfirstname": cardNameParts.first,
        "billlastname": cardNameParts.sublist(1).join(" "),
        "cardno": _cardCtrl.text.replaceAll(' ', ''),
        "expmonth": _expiryCtrl.text.split('/')[0],
        "expyear": "20${_expiryCtrl.text.split('/')[1]}",
        "cvcno": _cvvCtrl.text,
        ...
        "firstname": "NA",
        "lastname": "NA",
        "email": "test@skyrich.com",
        "_3dcheck": "N",
      };

      print("The Requested Payload: ${payload}");
      ...
      /// Final result
      final result = data["Capture"] != null
          ? {
              "responsekey": data["Capture"]["responsekey"],
              "amount": data["Capture"]["amount"],
              "reference": data["Capture"]["reference"],
              "description": data["Capture"]["description"],
              "currency": data["Capture"]["currency"],
              "errorcode": data["Capture"]["status"]?["errorcode"] ?? "",
              "errordesc": data["Capture"]["status"]?["errordesc"] ?? "",
            }
          : {
              ...
              "errordesc": data["status"]?["errordesc"] ?? "",
            };
      ...
      final fallback = LatpayDefaultResult.captureError(
        errorCode: "90",
        errorDesc: e.toString(),
      );

      LatpayPaymentCallback.onPaymentCompleted?.call(fallback);
      Navigator.pop(context);
    }
  }
  */
  ...
  Future<void> _processPayment() async {
    try {
      ...
      final cardNameParts = _nameCtrl.text.trim().split(RegExp(r'\s+'));
      final firstName = cardNameParts.first;
      final lastName = cardNameParts.sublist(1).join(" ");
      ...
      ///3D Secure Required
      if (data["VBVURL"] != null) {
        /// TODO: open WebView for 3DS
        /// After completion, backend should call callback
        return;
      }

      /// Non-3DS or Completed Capture
      ...
      LatpayPaymentCallback.onPaymentCompleted?.call(errorResult);
      Navigator.pop(context);
    }
  }
  */
